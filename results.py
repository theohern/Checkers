...
Random = load_model("random.keras")
Minmax = load_model("minmax.keras")
Itself = load_model("itself.keras")


def playRandomMinMax(Bot1="random", Bot2="random", verbose=False, GetFeatures=False, ArrayFeatures=[]):
    end2 = 0
    ...
    while True :
        ...
        if (player == 1):
            ...
            if game.CompareBoard(a, game.board) :
                ...
                print(game.board)
                break
            if GetFeatures : ArrayFeatures.append(game.GetFeatures(player))
            if verbose :
                print(move)
...
def CompareBots(player1, player2, NumberPlays):
    ...
    plt.title('Random (White) against Trainer (Black)')
    plt.show()

if "__main__" == __name__ :
    CompareBots(player1="TMinmax", player2="Titself", NumberPlays=10)